...
class TrtYOLOv5:
    def __init__(self, engine_file_path, input_shape=(640, 640),
        ...
        # model.model[-1].anchors (네가 보낸 값 그대로)
        ...
        # === 동적 입력 엔진인 경우 입력 shape 고정 설정 ===
        # (엔진이 explicit batch + -1 포함된 형태라면 필요)
        try:
            ...
            if -1 in shape:
                ...
                rospy.loginfo(f"[TrtYOLOv5] 동적 입력 → 고정 shape 설정: {(1, 3, self.input_h, self.input_w)}")
        except Exception as e:
            rospy.logwarn(f"[TrtYOLOv5] binding shape 설정 중 경고: {e}")
    ...
    def infer(self, image):
        ...
        # 전체 스케일 concat
        pred = torch.cat(decoded, dim=1)  # (1, N, no)
...
def capture_video_and_detect(width=640, height=480, fps=30, opt=None):
    global detect_start, detect_stop, d2t
    ...
    # ⭐ PyTorch 코드와 동일한 타임아웃 로직
    TIMEOUT_SECONDS = 4.0  # 4초 제한
    detection_successful = False
    last_detection_time = time.time()

    rospy.loginfo(f" 해상도: {width}x{height}, FPS: {fps}")

    try:
        while not rospy.is_shutdown():
            current_time = time.time()

            # 마지막으로 감지 성공했을 때 시간을 저장
            if detection_successful:
                last_detection_time = current_time

            # ⭐ PyTorch 버전과 동일: 일정 시간 동안 감지가 없으면 d2t로 신호, detect_stop = 1
            ...
            # 다음 루프에서 "타임아웃 체크"를 위해 일단 False로 두고,
            # 실제로 det 있을 때만 True로 되돌림 (PyTorch 코드와 동일)
            detection_successful = False
            ...
            # ⭐ PyTorch 코드와 동일:
            # detect_stop == 1이면 타임아웃이든 어떤 이유든 "감지 성공으로 간주"해서
            # 타임아웃이 재발동하지 않도록 detection_successful = True
            if detect_stop:
                detection_successful = True

            # task 노드에서 detect_stop = 0 으로 바꿔줄 때까지 대기
            while detect_stop:
                rospy.sleep(0.1)
