...
#include "open_manipulator_p_master_slave/open_manipulator_p_master_slave.h"

OpenManipulatorPMasterSlave::OpenManipulatorPMasterSlave(std::string usb_port, std::string baud_rate)
: node_handle_(""),
  priv_node_handle_("~"),
...
OpenManipulatorPMasterSlave::~OpenManipulatorPMasterSlave()
{
  delete actuator_;
  delete tool_;
...
void OpenManipulatorPMasterSlave::initServiceClient()
{
  goal_joint_space_path_client_ = node_handle_.serviceClient<open_manipulator_msgs::SetJointPosition>("goal_joint_space_path");
  goal_tool_control_client_ = node_handle_.serviceClient<open_manipulator_msgs::SetJointPosition>("goal_tool_control");
}

void OpenManipulatorPMasterSlave::initOpenManipulator(STRING usb_port, STRING baud_rate, double service_call_period)
{
  /*****************************************************************************
...
void OpenManipulatorPMasterSlave::syncOpenManipulator(bool recorded_state)
{
...
bool OpenManipulatorPMasterSlave::setJointSpacePath(double path_time, std::vector<double> set_goal_joint_position)
{
  ...
  // OMX 4DOF -> OMP 6DOF
  srv.request.joint_position.joint_name.push_back("joint5");
  srv.request.joint_position.joint_name.push_back("joint6");  
  srv.request.joint_position.position.push_back(0.0);
  srv.request.joint_position.position.push_back(0.0);
  double temp = srv.request.joint_position.position.at(3);
  srv.request.joint_position.position.at(3) = 0.0;
  srv.request.joint_position.position.at(4) = temp;
...
bool OpenManipulatorPMasterSlave::setToolPath(double set_goal_tool_position)
{
  double tool_value;
  ...
  // OMX Gripper -> OMP Gripper
  double temp = srv.request.joint_position.position.at(0);
  srv.request.joint_position.position.at(0) = (temp + 0.010) / 0.020 * 1.1;
...
void OpenManipulatorPMasterSlave::publishCallback(const ros::TimerEvent&)
{
  static int count = 0;
  if (!(count % 5))
  {
...
void OpenManipulatorPMasterSlave::setModeState(char ch)
{
  if (ch == '1')
  {
    syncOpenManipulator(false);
...
void OpenManipulatorPMasterSlave::printText()
{
  system("clear");
...
bool OpenManipulatorPMasterSlave::kbhit()
{
  termios term;
  tcgetattr(0, &term);
...
{
  // Init ROS node
  ros::init(argc, argv, "open_manipulator_p_master_slave");
  ...
  OpenManipulatorPMasterSlave open_manipulator_p_master_slave(usb_port, baud_rate);

  ros::Timer publish_timer = node_handle.createTimer(ros::Duration(open_manipulator_p_master_slave.getServiceCallPeriod()), &OpenManipulatorPMasterSlave::publishCallback, &open_manipulator_p_master_slave);
  ros::spin();
